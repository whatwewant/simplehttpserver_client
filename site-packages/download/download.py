...
def download_url(url, absolute_file_name, number, log, percent):
    support_accept_range = requests.head(url).headers.get('Accept-Range', None)
    if not support_accept_range:
        return download_log_without_range(url, absolute_file_name, number, log, percent)
    return download_log_accept_range(url, absolute_file_name, number, log, percent)

def download_log_without_range(url, absolute_file_name, number, log, percent):
    '''
        download without range
    ...
    while True:
        ...
        # Download Speed
        download_speed = file_size_dl / float(end_second - start_second)
...
def download_log_accept_range(url, absolute_file_name, number, log, percent):
    '''
        download support Accept-Range
        return tuple(file_target_size, file_real_size)
    '''
    file_name = absolute_file_name
